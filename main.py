...
with open(path.join(current_dir,"Menu_names.json"),"r") as file:
        Working_Menu_Names =  jsonLoad(fp = file)
    
with open(path.join(current_dir,"Parts_Names.json"),"r") as file:
    Working_Parts_Names =  jsonLoad(fp = file)

    
with open(path.join(current_dir,"Part_descriptions.json"),"r") as file:
    Working_Part_Desc =  jsonLoad(fp = file)

menu_and_partnames_list:list = []
name_dict ={}
for menukey,menuvalue in Working_Menu_Names.items():
    name_dict[menukey] = {}
...
helper_dict =    {k:dict.fromkeys(v,"") 
                      for k,v in Working_Parts_Names.items()
...
Working_Names_reversed = {}
for key,value in Working_Menu_Names.items():
    for element in value:
        Working_Names_reversed[element] = key if element not in all_code_exception_list else ""



def Workingcode_to_Dict(input:str)->dict[dict[str]]:
    ...
    for i in operant_list:
        ...
        part_name ="Error in Workingcode_to_dict"
        ...
        ####Testend
            ...
            match alpha:
                ...
                case "r"|"m"|'t'|'s'|'a'|'b'|'c'|'d'|'w'|'x'|'e'|'y'|'z'|'v'|'f':
                    ...
                    #print("case1 :",len(name_dict[part_name]) == len(Working_Parts_Names[alpha]))
                case  'xm'|'xr':
                    #form = xm000.---
                    part_name = alpha + num
                    ...
                    splitter_help_dict = {part_name : dict(zip(helper_dict[alpha].keys(),splitBy_dot_List))}
                    #print("case2 :",len(name_dict[part_name]) == len(Working_Parts_Names[alpha])) 
                ...
                case 'dh'|'da':
                    ...
                    #print("case3 :",len(name_dict[part_name]) == len(Working_Parts_Names[alpha])) 
                case _:
                    part_name = alpha
...
"""            
            ...
            #print(alpha,';',operant_dict[Working_Names_reversed[alpha]][alpha])
        
        list_of_parts_to_pop.remove([Working_Names_reversed[alpha],part_name]) if [Working_Names_reversed[alpha],part_name] in list_of_parts_to_pop else ""
        list_of_dicts_to_use.append(splitter_help_dict) 
        update_nested_dict(splitter_help_dict,[Working_Names_reversed[alpha],part_name],operant_dict)
        #print(operant_dict[Working_Names_reversed[alpha]][part_name]) if part_name == "da" else ""
...
def dict_to_WorkingCode(input:dict[dict[dict[str]]])->str:
        operant_dict:dict[dict[dict[str]]] = input.copy()
        
        Version     = operant_dict["ver"]["ver"]["ver"]
        del operant_dict["ver"]
        if "importObjects" in operant_dict and operant_dict["importObjects"] != "":
...
class WorkingCodeCovertor_Object:
    ...
    def __init__(self,compare:str,names_list:list[str],convert_list:list[str]) -> None:
        ...
        for index,name in enumerate(names_list):
            if bool(name) and convert_list[index] != "":
                print(name)
                self.convertdict[name] = Workingcode_to_Dict(convert_list[index])
                       
            else:
                print(index," skipped due to no name." )
        
        self.compare:str = compare
        print("compare_dict")
        self.comparedict:dict = Workingcode_to_Dict(self.compare)
       

        with open(path.join(current_dir,"Menu_names.json"),"r") as file:
            self.Working_Menu_Names =  jsonLoad(fp = file)
        

        with open(path.join(current_dir,"Parts_Names.json"),"r") as file:
            self.Working_Parts_Names =  jsonLoad(fp = file)
    ...
    def convert(self):
        ...
        convert_object_labels:list[list[str,str,str]] = list()
       ...
       # print(partOptions)
        ...
        def regsearch(Options_list:list[str],list_length:int,inputname:str = "menu",desc_dict:dict = None):    
            menu_excluded = []
            Options = Options_list[:]
            Options_clean = list(map(itemgetter(list_length),Options))
            if desc_dict !=None:
                temp_list =[]
                for index,key in enumerate(Options_clean):
                    key = key[:2] if str(key[:2]).isalpha() else key[0]
                    if index % 6 != 0:
                        temp_list.append(f"{key}: {desc_dict[key]}" + " " * (20-len(f"{key}: {desc_dict[key]}")))
                    else:
                        print(temp_list)
                        temp_list.clear()
                        temp_list.append(f"{key}: {desc_dict[key]}" + " " * (20-len(f"{key}: {desc_dict[key]}")))
...
Choose option by index(integer only): """)
        ...
        part_exclude,a= regsearch(partOptions,1,"part",Working_Part_Desc)
        del a
        print(part_exclude)
        ...
        if ribon_Hair_belt_Choice.isalpha():
            ribon_Hair_belt_Choice = True if ribon_Hair_belt_Choice.lower() == "y" else False   
            if ribon_Hair_belt_Choice == True:
                color_options:list = [[menukey,partkey,labelkey] 
                              for menukey,partkey,labelkey in label_name_dict 
                              if labelkey.find('color') != -1 
                              if labelkey not in label_exclude
                              if menukey in ["Ribon","Belt","HairEx"]] 
        
                color_changer, a = regsearch(color_options,2,"color")
                del a
                print(color_changer)    
        ...
        for key in self.convertdict:
            ...
            list_of_strings.append(dict_to_WorkingCode(convert_objects_dict[key]))
...
Options: 
{list(self.Working_Parts_Names)[:10]}   
{list(self.Working_Parts_Names)[10:23]}
{list(self.Working_Parts_Names)[23:30]}
{list(self.Working_Parts_Names)[30:41]}
""")    

        else:
            for index in tablist:
                print(f"{self.Working_Menu_Names[index]}:{[self.Working_Parts_Names[i] for i in self.Working_Menu_Names[index]]}")



try:
    with open(path.join(current_dir,"Loader and dumper.csv"),"r",newline="") as f:
...
working_Object = WorkingCodeCovertor_Object(convertor,names,codes)


output = working_Object.convert()

#[print(y,":",x,":",x==y) for x,y in zip(working_Object.convertdict["SiegeGolem"]["BodyHeight"]["ca"].keys(),working_Object.Working_Parts_Names["ca"])]
#print(dict_to_WorkingCode(output))


#print(dict_to_WorkingCode(working_Object.comparedict))
