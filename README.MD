...
The main.py file automatically converts aplhpa.num pattern to a reference alpha.num pattern in text. You can exclude parts of the pattern that you want preserved. For example, if you want to copy an eyecolor  but want to preserve the rest  of the sequnces being changed.

Sequence = Format: aa.000.000.000.000_ab_....._fv1.0 

Opertaion 1) Original   aa23.100.222.133.666
          2) Reference  aa1.0.0.0.0
          3) Change 1st and 3rd numeral to reference
          4) Output     aa1.100.0.133.666
***Components***
...
***How to use
1) Open the "Loader and dumper.csv".Copy the sequnces you want to convert under the **Input column**, below the headings.
...
2) Next, paste the string containing the squence you want to convert to the place shown (E2 cell in loader and dumper). For ease, I recommend exporting only the relevant parts of the sequnece string. *For example*, If I only want to change the eye color of the characters, I will select the Eye tab and export only that.
...
**Notes**
Please give me critcism. From code improvements to advice.

Special Thanks to KisekaeJade for the explaination of how the Save system works.
